...
export class NzUploadComponent implements OnInit, OnChanges, OnDestroy {
    ...
    if (this.nzSize > 0 && filters.findIndex(w => w.name === 'size') === -1) {
      filters.push({
        ...
        fn: (fileList: UploadFile[]) =>
          fileList.filter(w => w.size / 1024 <= this.nzSize)
      });
    }
    if (
      this.nzFileType &&
      this.nzFileType.length > 0 &&
  ...
  onRemove = (file: UploadFile): void => {
    ...
    file.status = 'removed';
    ((this.nzRemove
      ? this.nzRemove instanceof Observable
        ? this.nzRemove
        : of(this.nzRemove(file))
      : of(true)) as Observable<any>)
  ...
  setClassMap(): void {
    ...
    if (this.nzType === 'drag') {
      subCls = [
        this.nzFileList.some(file => file.status === 'uploading') &&
          `${this.prefixCls}-drag-uploading`,
