...
export class NzTableComponent implements OnInit, AfterViewInit {
  ...
  @ContentChildren(NzThComponent, { descendants: true })
  ...
  checkPageIndexBounding(): void {
    if (this.nzFrontPagination) {
      ...
      const pageIndex = !this.nzPageIndex
        ? 1
        : this.nzPageIndex > maxPageIndex ? maxPageIndex : this.nzPageIndex;
  ...
  generateSyncDisplayData(): void {
    this.data = this.syncData.slice(
      (this.nzPageIndex - 1) * this.nzPageSize,
      this.nzPageIndex * this.nzPageSize
    );
    this.nzCurrentPageDataChange.emit(this.data);
  }
  ...
  setScrollPositionClassName(): void {
    if (this.tableBodyElement && this.nzScroll && this.nzScroll.x) {
      if (
        this.tableBodyElement.nativeElement.scrollWidth ===
          this.tableBodyElement.nativeElement.clientWidth &&
        this.tableBodyElement.nativeElement.scrollWidth !== 0
      ) {
