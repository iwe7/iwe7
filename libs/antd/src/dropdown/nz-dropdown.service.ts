...
export class NzDropdownService {
  ...
  private createOverlay($event: MouseEvent): OverlayRef {
    ...
    const originPoint = {
      originX: defaultPosition.originX,
      originY: defaultPosition.originY
    };
    const overlayPoint = {
      overlayX: defaultPosition.overlayX,
      overlayY: defaultPosition.overlayY
    };
    const fakeElementRef = { nativeElement: this.locatePoint };
    this.positionStrategy = this.overlay
      .position()
      .connectedTo(fakeElementRef, originPoint, overlayPoint);
  ...
  private handlePositionChanges(strategy: ConnectedPositionStrategy): void {
    ...
    this.onPositionChangeSubscription = this.positionStrategy.onPositionChange.subscribe(
      data => {
        const position =
          data.connectionPair.overlayY === 'bottom' ? 'top' : 'bottom';
  ...
  private handleCloseEvent(overlayRef: OverlayRef): void {
    this.backdropClickSubscription = overlayRef
      .backdropClick()
      .subscribe(_ => this.instance.close());
    this.detachmentsSubscription = overlayRef
      .detachments()
  ...
  private setInstanceValue(
    instance: NzDropdownContextComponent,
  ...
  create(
    ...
    if (this.overlayRef && this.overlayRef.hasAttached()) {
      ...
      this.instance = this.overlayRef.attach(
        new ComponentPortal(NzDropdownContextComponent)
      ).instance;
