...
export class NzDropDownComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  set nzDisabled(value: boolean) {
    ...
    if (this._disabled) {
      this.renderer.setAttribute(
        this.nzOrigin.elementRef.nativeElement,
        'disabled',
        ''
      );
    } else {
  ...
  set nzPlacement(value: NzPlacement) {
    ...
    this.dropDownPosition =
      this.nzPlacement.indexOf('top') !== -1 ? 'top' : 'bottom';
    this.positions.unshift(POSITION_MAP[
      this.placement
  ...
  startSubscribe(observable$: Observable<boolean>): void {
    let $pre = observable$;
    if (this.nzClickHide) {
      const $menuItemClick = this.nzMenu.nzClick
        .asObservable()
        .pipe(mapTo(false));
      $pre = $pre.pipe(merge($menuItemClick));
    }
    const final$ = $pre.pipe(
