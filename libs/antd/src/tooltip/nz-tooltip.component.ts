...
export class NzToolTipComponent {
  ...
  set nzPlacement(value: string) {
    if (value !== this._placement) {
      ...
      this._positions.unshift(POSITION_MAP[
        this.nzPlacement
  ...
  onPositionChange($event: ConnectedOverlayPositionChange): void {
    for (const key in POSITION_MAP) {
      if (
        JSON.stringify($event.connectionPair) ===
        JSON.stringify(POSITION_MAP[key])
      ) {
  ...
  private isContentEmpty(): boolean {
    ...
    return this.nzTemplate || this.nzContent
      ? false
      : this.nzTitle === '' || this.nzTitle == null; // Pity, can't detect whether nzTemplate is empty due to can't get it's content before shown up
  }
}
