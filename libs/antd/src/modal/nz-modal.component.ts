...
export class NzModalComponent<T = any, R = any> extends NzModalRef<T, R>
  ...
  nzGetContainer:
    | HTMLElement
    | OverlayRef
    | (() => HTMLElement | OverlayRef) = () => this.overlay.create(); // [STATIC]
  ...
  @Input()
  @Output()
  nzOnOk: EventEmitter<T> | OnClickCallback<T> = new EventEmitter<T>();
  @ViewChild('autoFocusButtonOk', { read: ElementRef })
  autoFocusButtonOk: ElementRef; // Only aim to focus the ok button that needs to be auto focused
  ...
  @Input()
  @Output()
  nzOnCancel: EventEmitter<T> | OnClickCallback<T> = new EventEmitter<T>();
  ...
  ngOnInit(): void {
    ...
    if (this.isModalButtons(this.nzFooter)) {
      ...
      this.nzFooter = this.formatModalButtons(this.nzFooter as Array<
        ModalButtonOptions<T>
      >);
    }

    // Place the modal dom to elsewhere
    this.container =
      typeof this.nzGetContainer === 'function'
        ? this.nzGetContainer()
        : this.nzGetContainer;
  ...
  private onClickOkCancel(type: 'ok' | 'cancel'): void {
    ...
    if (trigger instanceof EventEmitter) {
      ...
      const caseClose = (doClose: boolean | void | {}) =>
        doClose !== false && this.close(); // Users can return "false" to prevent closing by default
      if (isPromise(result)) {
  ...
  private getButtonCallableProp(
    options: ModalButtonOptions<T>,
    prop: string
  ): {} {
  ...
  private formatModalButtons(
    buttons: Array<ModalButtonOptions<T>>
  ...
  private updateTransformOrigin(): void {
    ...
    if (lastPosition) {
      this.transformOrigin = `${lastPosition.x -
        modalElement.offsetLeft}px ${lastPosition.y -
  ...
  private changeBodyOverflow(): void {
    ...
    if (openModals.length) {
      this.renderer.setStyle(
        this.document.body,
        'padding-right',
        `${this.nzMeasureScrollbarService.scrollBarWidth}px`
      );
