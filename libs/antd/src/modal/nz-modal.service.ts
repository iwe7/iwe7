...
export class ModalBuilderForService {
  ...
  private createModal(): void {
    ...
    this.modalRef = this.overlayRef.attach(
      new ComponentPortal(NzModalComponent)
    );
  }
}
...
export class NzModalService {
  ...
  create<T>(options: ModalOptionsForService<T> = {}): NzModalRef<T> {
    ...
    const modalRef = new ModalBuilderForService(
      this.overlay,
      options
    ).getInstance(); // NOTE: use NzModalComponent as the NzModalRef by now, we may need archive the real NzModalRef object in the future

    return modalRef;
  }

  confirm<T>(
    options: ModalOptionsForService<T> = {},
    confirmType: ConfirmType = 'confirm'
  ): NzModalRef<T> {
    if ('nzFooter' in options) {
      this.logger.warn(
        `The Confirm-Modal doesn't support "nzFooter", this property will be ignored.`
      );
    }
