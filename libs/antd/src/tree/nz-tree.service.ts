...
export class NzTreeService {
  ...
  setSelectedNodeList(node: NzTreeNode, isMultiple: boolean): void {
    if (isMultiple) {
      const sIndex = this.selectedNodeList.findIndex(
        cNode => node.key === cNode.key
      );
  ...
  setCheckedNodeList(node: NzTreeNode): void {
    if (
      node.isChecked &&
      this.checkedNodeList.findIndex(
        cNode => node.key === cNode.key && node.title === cNode.title
      ) === -1
    ) {
      this.checkedNodeList.push(node);
    }
    const removeChild = (rNode: NzTreeNode) => {
      const rIndex = this.checkedNodeList.findIndex(
        cNode => rNode.key === cNode.key && rNode.title === cNode.title
      );
      if (rIndex > -1) {
    ...
    this.rootNodes.forEach((rNode: NzTreeNode) => {
      const loopNode = (lNode: NzTreeNode) => {
        const cIndex = this.checkedNodeList.findIndex(
          cNode => lNode.key === cNode.key && lNode.title === cNode.title
        );
        if (lNode.isChecked) {
          if (cIndex === -1) {
    ...
    if (parentNode) {
      if (
        parentNode.children.every(
          child =>
            child.isDisabled ||
            child.isDisableCheckbox ||
            (!child.isHalfChecked && child.isAllChecked)
        )
      ) {
        ...
        parentNode.children.some(
          child => child.isHalfChecked || child.isAllChecked
        )
      ) {
  ...
  dropAndApply(targetNode: NzTreeNode, dragPos: number = -1): void {
    ...
    if (isSelectedRootNode) {
      isSelectedRootNode
        .getChildren()
        .splice(isSelectedRootNode.getChildren().indexOf(this.selectedNode), 1);
    } else {
    ...
    switch (eventName) {
      ...
      case 'click':
        Object.assign(emitStructure, {
          selectedKeys: this.getSelectedNodeList()
        });
        break;
      case 'check':
        Object.assign(emitStructure, {
          checkedKeys: this.getCheckedNodeList()
        });
        break;
      case 'search':
        Object.assign(emitStructure, {
