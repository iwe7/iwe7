...
export class NzTabSetComponent
  ...
  set nzAnimated(value: NzAnimatedInterface | boolean) {
    ...
    this.inkBarAnimated =
      ...
      (this.nzAnimated as NzAnimatedInterface).inkBar === true;
    this.tabPaneAnimated =
      this.nzAnimated === true ||
      (this.nzAnimated as NzAnimatedInterface).tabPane === true;
  }
  ...
  @Output()
  nzSelectChange: EventEmitter<NzTabChangeEvent> = new EventEmitter<
    NzTabChangeEvent
  >(true);
  ...
  setClassMap(): void {
    const classMap = {
      [this.prefixCls]: true,
      [`${this.prefixCls}-vertical`]:
        this.nzTabPosition === 'left' || this.nzTabPosition === 'right',
      [`${this.prefixCls}-${this.nzTabPosition}`]: this.nzTabPosition,
      [`${this.prefixCls}-no-animation`]:
        this.nzAnimated === false ||
        (this.nzAnimated as NzAnimatedInterface).tabPane === false,
  ...
  ngAfterContentChecked(): void {
    ...
    const indexToSelect = (this._indexToSelect = Math.min(
      this.listOfNzTabComponent.length - 1,
  ...
  removeTab(value: NzTabComponent): void {
    this.listOfNzTabComponent.splice(
      this.listOfNzTabComponent.indexOf(value),
      1
    );
  }
